...
func (acpt *UnixAcceptor) Start() error {
	if actp.IsListening {
		return errors.New("UnixAcceptor is already listening, cannot Start twice")
	}
	os.Remove(acpt.path)
...
func (acpt *UnixAcceptor) Stop() error {
	defer func() {
...
func NewUnixAccpetor(path string) (*UnixAcceptor, error) {
